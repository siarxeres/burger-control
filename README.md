...
- ⬜ **[BUG ABERTO] Editar orçamento não funciona (descoberto 27/06/2026)** — na tela Orçado vs Realizado, o botão de editar (lápis ✏️) não permite editar: no desktop o modal não abre (Console mostra "Uncaught SyntaxError: (índice):1" que persiste mesmo após o SW estar OK — ou seja, não é o problema do Service Worker já corrigido); no mobile a coluna do lápis nem aparece (cortada no layout responsivo da tabela). O `index.html` passa no `node --check` (sem erro de sintaxe estático) e o arquivo servido em produção é byte-a-byte idêntico ao repo — logo é provável erro de execução (runtime), não de sintaxe estática. **A investigar:** função `openModalOrcamento()` e o caminho de salvar `salvarOrcamento()`, além do CSS responsivo da coluna de ações da tabela. **Nota:** esse bug provavelmente impediu a validação real do conserto da cota (`a149f1f`).
...
Ver item **[BUG ABERTO] Editar orçamento não funciona** na seção Pendências (descoberto 27/06/2026).
