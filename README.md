...
- ⬜ **Etapa B — remover recorrência de Lançar Despesa** — a recorrência foi adicionada ao orçamento (Etapa A, concluída). Falta removê-la do formulário de Lançar Despesa, onde ela está conceitualmente errada (despesa = realização/fato consumado, não planejamento). Confirmado por query SQL: zero despesas recorrentes no banco (`recorrente=true` → 0 registros), então a remoção é segura, sem dado a preservar nem usuário a impactar. Branch `recorrencia-por-periodos` (commit `d9ddd9c`, refinamento de UX feito na despesa) NÃO foi mergeada e fica de referência.
