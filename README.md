...
- ⬜ **Tradução de linguagem — Rodada 2 / baixa visibilidade (em andamento)** — fechar a coerência dos pontos internos ainda em jargão que "brigam" com o menu traduzido na R1. Alvos: títulos de tela no topbar (objeto `titles` em `index.html`: `"Centros de Custo"` → `"Meus gastos"`, `"DRE — Demonstrativo de Resultado"` → `"Resumo do mês"`); cabeçalhos de coluna na tabela de despesas (`"CENTRO DE CUSTO"` / `"CC"` → `"Tipo de gasto"`); aba e filtro do Orçado vs Realizado; rótulo do campo de lançamento/edição de despesa; botões e toasts internos do DRE; mensagens de estado vazio. Mesma regra da R1: só texto de exibição, nunca valores de dado (nomes de CC gravados, comparações, IDs, `value` de `<option>`). Termo único de CC definido: `"Meus gastos"` (menu/título de tela) e `"Tipo de gasto"` (campo, coluna e filtro).
