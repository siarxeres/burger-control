...
- ✅ **Orçamento — memória de cálculo opcional + observações (02/07/2026)** — branch `orcamento-memoria-calculo`, commit `034d9a6`. No preview, pendente merge na main. O modal "Definir Orçamento" ganhou uma seção opcional de memória de cálculo para o dono montar o valor orçado do jeito que pensa (por consumo diário), em vez de só digitar o total. **Quatro campos novos:** Qtd. por dia, Valor unitário, Total do dia (somente leitura, calculado = qtd × unitário) e Dias trabalhados no mês; o Valor Orçado se autopreenche = total do dia × dias trabalhados, mas continua editável à mão. Campo Observações (textarea livre) entre o valor orçado e o Salvar. **Tudo opcional:** quem não usar a calculadora digita o valor direto como antes. **Persistência:** ao reabrir para editar, os campos voltam preenchidos. **Schema:** 4 colunas novas na tabela `orcamentos` (`qtd_dia` NUMERIC, `valor_unitario` NUMERIC, `dias_trabalhados` INTEGER, `observacoes` TEXT), todas opcionais/nullable — aplicadas manualmente no Supabase via `ALTER TABLE ADD COLUMN IF NOT EXISTS`. Coluna de valor existente confirmada como `valor_orcado`. Recorrência, cota (`podeLancar`), navegação mês/ano e excluir: zero alteração. Alinhado à regra do produto (orçamento = planejamento). Validado no desktop (autocálculo 15×10×26 = 3.900) e no mobile (persistência ao reabrir OK; campo `type="number"` aceita decimal, exibindo com ponto — ex.: `3.5` — em vez de vírgula: cálculo correto, atrito cosmético registrado como ajuste futuro opcional). Mergeado na main.
- ✅ **Aba Receitas — "botão de despesa indevido" investigado: não é bug (03/07/2026)** — auditoria confirmou que o botão "+ Lançar" na tela de Receitas não pertence a ela: é o botão global do topbar (linha 689), presente em todas as telas por design, disparando `showPage('lancar')` — mesmo atalho da sidebar e dos CTAs do dashboard. Não há botão de despesa solto em `#page-receitas`. O "+ Nova Receita" (botão correto da tela) permanece. Decisão: deixar como está — atalho global consistente, não elemento indevido; escondê-lo só em Receitas exigiria lógica condicional sem ganho real. Pendência encerrada.
- ✅ **Tradução de linguagem — Rodada 3 / conteúdo do Resumo do Mês (03/07/2026)** — branch `traducao-linguagem-r3`, merge --no-ff na `main`, commit `3fb430c`, merge `eeb8f5d`. Traduzidos os rótulos de exibição da tabela do Resumo do Mês (`renderDRE`) do jargão contábil para a língua do ME: "RECEITAS TOTAIS"→"Tudo que entrou", "CUSTOS DE PRODUÇÃO (CMV)"→"Quanto custou o que você vendeu", "MARGEM DE CONTRIBUIÇÃO"→"O que sobrou pra pagar as contas", "DESPESAS TOTAIS"→"Contas do mês", "Despesas Fixas/Variáveis"→"Contas que não mudam/que variam", "RESULTADO OPERACIONAL (EBITDA)"→"Resultado antes dos impostos" (subtexto redundante removido), "RESULTADO LÍQUIDO"→"Quanto sobrou de verdade", "PONTO DE EQUILÍBRIO"→"Quanto precisa vender pra não ter prejuízo" (versão curta "Quanto vender pra não ter prejuízo" no card do rodapé), "Meta diária necessária"→"Quanto vender por dia", cabeçalho "% Receita"→"% do que entrou". **Decisões:** substituir o jargão; CSV mantido com termos técnicos (`exportDRE` intocado — documento do contador); DASN preservado (nomenclatura fiscal oficial). **Salvaguarda:** traduzido só exibição — listas de classificação (`CMV_CATS`, `FIXAS_CATS`, `CC_PRODUCAO`, `IMPOSTO_CATS`), valores `natureza`, `alertPlano('Ponto de Equilíbrio')` e cálculos intocados. 14 trocas de texto, zero lógica alterada. Validado em preview antes do merge.
...
- ⬜ **Categorias guiadas no orçamento — natureza contábil + condução do usuário (frente registrada 03/07/2026)** — evolução da "frente arquitetural grande" (alinhamento Orçamento × Despesa × Resumo). Objetivo: fazer orçamento e despesa usarem os dois níveis (centro de custo + categoria) e o Resumo classificar corretamente, sem sobrecarregar o dono com parametrização. Desenho acordado: (1) Categoria vira dado real — tabela categorias no Supabase, ligada ao centro de custo, com CRUD no admin; cada categoria carrega um campo natureza (CMV / Custo Fixo / Custo Variável / Imposto). (2) Catálogo default de fábrica — categorias comuns de food service já vêm prontas com centro + natureza definidos (Carne→Cozinha→CMV, Aluguel→Ocupação→Fixo, Taxa cartão→Vendas→Variável, DAS→Impostos→Imposto, etc.); o dono não parametriza o que nunca varia. (3) Condução interativa — só quando o dono cria algo fora do catálogo, o app pergunta em linguagem simples ("isso é ingrediente que vira comida / conta fixa / taxa sobre vendas / imposto?") e traduz para a natureza internamente, sem expor "CMV". (4) O Resumo do Mês passa a classificar por natureza (não por listas de nomes hardcoded), e o casamento orçado × realizado passa a comparar o par centro+categoria. Princípio (definido pelo dono): sistema não pode ficar grande/burocrático — deve ser usual e rico em informação; defaults resolvem 80-90% sem toque do usuário. Tratar como projeto faseado, mapa fechado antes da obra — mexe em renderDRE (cálculo de lucro), sensível. Registrado como não-urgente: priorizar frentes que destravam monetização primeiro.
...
- ⬜ **Tradução — resíduo leve no DASN (opcional, baixa prioridade)** — a tela Declaração Anual tem jargão leve não tratado na R3: "Receita Bruta Total" e o cabeçalho "Natureza" (coluna de enquadramento). Não são termos oficiais obrigatórios como "DASN"/"DAS", então poderiam ser suavizados no futuro, mas sem urgência. Relatórios IA já está limpo (não requer ação).
...
- ⬜ **Pastas duplicadas do projeto na máquina local — a investigar (registrado 02/07/2026)** — em `OneDrive\Apps\` existem múltiplas pastas parecidas: `burger-control` (a correta, produção), `burger-control-final`, e um `burger-control.zip` (backup de 18/05/2026). Risco: abrir a pasta errada no VS Code faz o Claude Code editar o projeto errado, e o trabalho não chega à produção. Cuidado imediato: sempre confirmar que o caminho aberto no VS Code é exatamente `...\OneDrive\Apps\burger-control` (sem sufixo) antes de trabalhar. A tratar: investigar o conteúdo/propósito de cada pasta duplicada e limpar as obsoletas (com backup antes de apagar — dados locais, decidir com cuidado). Relacionado: branch `Dev` (`6a0cb62`) também aguarda investigação.
